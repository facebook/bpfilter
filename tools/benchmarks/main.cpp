...
{
    // meta.flow_hash is only supported on TC hooks
...
void single_rule__meta_flow_probability_tc(::benchmark::State &state)
{
    ...
    state.counters["nInsn"] = prog.nInsn();
    state.SetLabel("1 rule, meta.flow_probability, TC");
}
BENCHMARK(single_rule__meta_flow_probability_tc);

void single_rule__meta_flow_probability_cgroup(::benchmark::State &state)
{
    // meta.flow_probability is supported on CGROUP hooks
    Chain chain("bf_benchmark", BF_HOOK_CGROUP_INGRESS, BF_VERDICT_ACCEPT);
    ...
    // CGROUP_DROP = 0, CGROUP_ACCEPT = 1
    while (state.KeepRunningBatch(::bf::progRunRepeat)) {
        auto stats = prog.run(::bf::pkt_local_ip4_tcp);
        if (stats.retval != ::bf::CGROUP_DROP)
            state.SkipWithError("benchmark run failed");
    ...
    state.counters["nInsn"] = prog.nInsn();
    state.SetLabel("1 rule, meta.flow_probability, CGROUP");
}
BENCHMARK(single_rule__meta_flow_probability_cgroup);
