...
static void init_and_clean(void **state)
{
    ...
    {
        bf_vector vec = bf_vector_default(sizeof(int));
        int val = 42;
    ...
    {
        _clean_bf_vector_ bf_vector vec;
        bf_vector_init(&vec, sizeof(int));
    }
...
static void foreach(void **state)
{
    ...
    // Iterating over an empty vector is a no-op.
    bf_vector_foreach (&vec, elem0) {
        (void)elem0;
        ++count;
    }
