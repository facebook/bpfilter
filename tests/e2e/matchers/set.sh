...
# Empty set tests: empty sets are valid, rules referencing them are removed
bfcli ruleset set --dry-run --from-str "chain xdp BF_HOOK_XDP ACCEPT rule (ip4.saddr) in {} DROP" 2>&1 | grep -q "ignoring rule at index 0: references empty set"
bfcli ruleset set --dry-run --from-str "chain xdp BF_HOOK_XDP ACCEPT rule (ip4.saddr, ip4.proto) in {} DROP" 2>&1 | grep -q "ignoring rule at index 0: references empty set"
bfcli ruleset set --dry-run --from-str "chain xdp BF_HOOK_XDP ACCEPT
    set myset (ip4.saddr) in {}
    rule (ip4.saddr) in myset DROP
" 2>&1 | grep -q "ignoring rule at index 0: references empty set"
bfcli ruleset set --dry-run --from-str "chain xdp BF_HOOK_XDP ACCEPT
    rule (ip4.saddr) in { 192.168.1.1 } DROP
    rule (ip4.saddr) in {} ACCEPT
" 2>&1 | grep -q "ignoring rule at index 1: references empty set"
