 ...
 */
static int bft_matcher_test_setup(void **state)
{
    int r;
    (void)state;
 ...
 */
static int bft_matcher_test_teardown(void **state)
{
    int r;
...
{
    std::vector<uint8_t> payload(sizeof(uint16_t) * 2);
...
public:
        ...
        _meta {bf_matcher_get_meta(type)}
    {
        ...
        for (auto hook = static_cast<enum bf_hook>(0); hook < _BF_HOOK_MAX;
    ...
    static void _coverage_check(void **state)
    {
        ...
        for (const auto &[hook, op]: *check) {
            cm_print_error("missing functional test: %s x %s\n",
    ...
    int run()
    {
        ...
        tests.push_back(CMUnitTest {.name = "coverage",
                                    .test_func = _coverage_check,
                                    .setup_func = nullptr,
                                    .teardown_func = nullptr,
                                    .initial_state = (void *)&_check});
