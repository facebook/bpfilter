...
/**
 * Verify that icmpv6.type eq N matches a packet with ICMPv6 type N and
 ...
 */
static void icmpv6_type_eq(void **state)
{
    ...
    // ICMPv6 type=128 (echo request) should match -> DROP
    bft_assert_prog_run(
        "test_icmpv6_type", test->hook(),
        bft::Ethernet() /
            bft::IPv6 {.saddr = "2001:db8::1", .daddr = "2001:db8::2"} /
            bft::ICMPv6 {.type = 128, .code = 0},
        test->verdictDrop());

    // ICMPv6 type=129 (echo reply) should not match -> ACCEPT
    bft_assert_prog_run(
        "test_icmpv6_type", test->hook(),
        bft::Ethernet() /
            bft::IPv6 {.saddr = "2001:db8::1", .daddr = "2001:db8::2"} /
            bft::ICMPv6 {.type = 129, .code = 0},
        test->verdictAccept());
...
static void icmpv6_type_ne(void **state)
{
    ...
    // ICMPv6 type=129 should match -> DROP
    bft_assert_prog_run(
        "test_icmpv6_type", test->hook(),
        bft::Ethernet() /
            bft::IPv6 {.saddr = "2001:db8::1", .daddr = "2001:db8::2"} /
...
/**
 * Verify that icmpv6.type in {set} matches packets whose type is in
 * the set and does not match packets with types outside the set.
 */
static void icmpv6_type_in(void **state)
{
    auto *test = static_cast<MatcherTest *>(*state);

    auto set = bf::Set({BF_MATCHER_ICMPV6_TYPE});
    set << std::vector<uint8_t> {128} << std::vector<uint8_t> {129};
    ...
    // ICMPv6 type=128 is in set -> DROP
    bft_assert_prog_run(
        "test_icmpv6_type", test->hook(),
        bft::Ethernet() /
            bft::IPv6 {.saddr = "2001:db8::1", .daddr = "2001:db8::2"} /
    ...
    // ICMPv6 type=129 is also in set -> DROP
    bft_assert_prog_run(
        "test_icmpv6_type", test->hook(),
        bft::Ethernet() /
            bft::IPv6 {.saddr = "2001:db8::1", .daddr = "2001:db8::2"} /
    ...
    // ICMPv6 type=1 is not in set -> ACCEPT
    bft_assert_prog_run(
        "test_icmpv6_type", test->hook(),
        bft::Ethernet() /
            bft::IPv6 {.saddr = "2001:db8::1", .daddr = "2001:db8::2"} /
            bft::ICMPv6 {.type = 1, .code = 0},
        test->verdictAccept());

    bft_assert_counter_eq("test_icmpv6_type", 0, 2, -1);
}
