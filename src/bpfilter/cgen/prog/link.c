                ...
                struct bf_hookopts **hookopts, int prog_fd)
{
    ...
    if (name[0] == '\0')
        return bf_err_r(-EINVAL, "link name can't be empty");
...
static int _bf_link_update_nf(struct bf_link *link, int prog_fd)
{
    ...
    r = bf_bpf_link_create(prog_fd, 0, link->hook, 0, PF_INET,
                           opts.priorities[1]);
    if (r < 0)
        return bf_err_r(r, "failed to create nf_inet BPF link");
    new_inet_fd = r;

    r = bf_bpf_link_create(prog_fd, 0, link->hook, 0, PF_INET6,
                           opts.priorities[1]);
    if (r < 0)
        return bf_err_r(r, "failed to create nf_inet6 BPF link");
    new_inet6_fd = r;
...
int bf_link_update(struct bf_link *link, int prog_fd)
{
    ...
    case BF_FLAVOR_CGROUP:
        ...
        break;
    case BF_FLAVOR_NF:
        r = _bf_link_update_nf(link, prog_fd);
        break;
    default:
        return -ENOTSUP;
    }
