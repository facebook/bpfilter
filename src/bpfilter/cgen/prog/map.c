...
int bf_map_set_key_size(struct bf_map *map, size_t key_size)
{
    ...
    if (key_size == 0)
        return bf_err_r(-EINVAL, "map key size can't be zero");
...
int bf_map_set_value_size(struct bf_map *map, size_t value_size)
{
    ...
    if (value_size == 0)
        return bf_err_r(-EINVAL, "map value size can't be zero");
...
int bf_map_set_n_elems(struct bf_map *map, size_t n_elems)
{
    ...
    if (n_elems == 0)
        return bf_err_r(-EINVAL, "map number of elements can't be zero");
