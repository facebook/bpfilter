                                         ...
                                         const struct bf_matcher *matcher)
{
    ...
    size_t offset = bf_matcher_get_type(matcher) == BF_MATCHER_TCP_SPORT ?
                                          ...
                                          const struct bf_matcher *matcher)
{
    ...
    if (bf_matcher_get_op(matcher) == BF_MATCHER_ANY ||
        bf_matcher_get_op(matcher) == BF_MATCHER_ALL)
        return bf_cmp_bitfield(program, bf_matcher_get_op(matcher), flags,
                               BPF_REG_1);

    return bf_cmp_value(program, bf_matcher_get_op(matcher), &flags, 1,
                        BPF_REG_1);
}
