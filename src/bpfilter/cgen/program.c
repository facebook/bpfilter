                                     ...
                                     struct bf_rule *rule)
{
    ...
    case BF_VERDICT_NEXT: {
        int retcode;
        r = program->runtime.ops->get_verdict(rule->verdict, &retcode);
        if (r)
            return r;
        EMIT(program, BPF_MOV64_IMM(BPF_REG_0, retcode));
...
int bf_program_generate(struct bf_program *program)
{
    ...
    {
        int retcode;
        r = program->runtime.ops->get_verdict(chain->policy, &retcode);
        if (r)
            return r;
        EMIT(program, BPF_MOV64_IMM(BPF_REG_0, retcode));
        EMIT(program, BPF_EXIT_INSN());
