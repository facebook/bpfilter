                                          ...
                                          const struct bf_matcher *matcher)
{
    ...
    case BF_MATCHER_EQ:
        EMIT_FIXUP(program, BF_CODEGEN_FIXUP_NEXT_RULE,
                   BPF_JMP_IMM(BPF_JNE, BF_REG_1, htonl(flags), 0));
        break;
    case BF_MATCHER_NE:
        EMIT_FIXUP(program, BF_CODEGEN_FIXUP_NEXT_RULE,
                   BPF_JMP_IMM(BPF_JEQ, BF_REG_1, htonl(flags), 0));
        break;
    case BF_MATCHER_ANY:
        EMIT(program, BPF_ALU32_IMM(BPF_AND, BPF_REG_1, htonl(flags)));
        EMIT_FIXUP(program, BF_CODEGEN_FIXUP_NEXT_RULE,
    ...
    case BF_MATCHER_ALL:
        EMIT(program, BPF_ALU32_IMM(BPF_AND, BPF_REG_1, htonl(flags)));
        EMIT_FIXUP(program, BF_CODEGEN_FIXUP_NEXT_RULE,
                   BPF_JMP_IMM(BPF_JNE, BPF_REG_1, htonl(flags), 0));
        break;
    default:
                            ...
                            const struct bf_matcher *matcher)
{
    ...
    EMIT_FIXUP(program, BF_CODEGEN_FIXUP_NEXT_RULE,
               BPF_JMP_IMM(BPF_JNE, BF_REG_1, htonl(IPPROTO_TCP), 0));
