...
static const char *__func_get_name(const struct bpf_insn_cbs *cbs,
                                   const struct bpf_insn *insn, char *buff,
                                   size_t len)
{
...
static const char *__func_imm_name(const struct bpf_insn_cbs *cbs,
                                   const struct bpf_insn *insn, u64 full_imm,
...
static const char * const bpf_jmp_string[16] = {
    [BPF_JA >> 4] = "jmp",    [BPF_JEQ >> 4] = "==",    [BPF_JGT >> 4] = ">",
    [BPF_JLT >> 4] = "<",     [BPF_JGE >> 4] = ">=",    [BPF_JLE >> 4] = "<=",
    [BPF_JSET >> 4] = "&",    [BPF_JNE >> 4] = "!=",    [BPF_JSGT >> 4] = "s>",
    [BPF_JSLT >> 4] = "s<",   [BPF_JSGE >> 4] = "s>=",  [BPF_JSLE >> 4] = "s<=",
    [BPF_CALL >> 4] = "call", [BPF_EXIT >> 4] = "exit",
};

static void print_bpf_end_insn(bpf_insn_print_t verbose, void *private_data,
...
static void print_bpf_bswap_insn(bpf_insn_print_t verbose, void *private_data,
                                 const struct bpf_insn *insn)
{
    verbose(private_data, "(%02x) r%d = bswap%d r%d\n", insn->code,
            insn->dst_reg, insn->imm, insn->dst_reg);
}
...
{
    return insn->code == (BPF_ALU64 | BPF_MOV | BPF_X) &&
           insn->off == BPF_ADDR_PERCPU;
}

void print_bpf_insn(const struct bpf_insn_cbs *cbs, const struct bpf_insn *insn,
    ...
    if (class == BPF_ALU || class == BPF_ALU64) {
        if (BPF_OP(insn->code) == BPF_END) {
            ...
            verbose(cbs->private_data, "(%02x) %c%d %s %s%c%d\n", insn->code,
                    class == BPF_ALU ? 'w' : 'r', insn->dst_reg,
                    is_sdiv_smod(insn) ?
                        bpf_alu_sign_string[BPF_OP(insn->code) >> 4] :
        ...
        if (BPF_MODE(insn->code) == BPF_MEM)
            ...
            verbose(cbs->private_data,
                    ...
                    bpf_ldst_string[BPF_SIZE(insn->code) >> 3], insn->dst_reg,
        ...
        if (BPF_MODE(insn->code) == BPF_ABS) {
            ...
            verbose(cbs->private_data, "(%02x) r%d = %s\n", insn->code,
                    insn->dst_reg,
                    __func_imm_name(cbs, insn, imm, tmp, sizeof(tmp)));
        } else {
        ...
        if (opcode == BPF_CALL) {
            ...
            verbose(cbs->private_data, "(%02x) if %c%d %s %c%d goto pc%+d\n",
                    insn->code, class == BPF_JMP32 ? 'w' : 'r', insn->dst_reg,
                    bpf_jmp_string[BPF_OP(insn->code) >> 4],
                    class == BPF_JMP32 ? 'w' : 'r', insn->src_reg, insn->off);
        } else {
            verbose(cbs->private_data, "(%02x) if %c%d %s 0x%x goto pc%+d\n",
                    insn->code, class == BPF_JMP32 ? 'w' : 'r', insn->dst_reg,
                    bpf_jmp_string[BPF_OP(insn->code) >> 4], insn->imm,
                    insn->off);
        }
    } else {
