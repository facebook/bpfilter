...
struct bf_program
{
    ...
    /** Non-empty sets from `runtime.chain->sets`, grouped by key format.
     * Each group shares a single BPF map using bitmask values; a set's
     * position within a group is its bit index. Not serialized. */
    bf_list set_groups;
 ...
 *
 * Sets sharing the same key format are grouped together and share a single
 * BPF map. Each set's elements are stored in the map with a bitmask value
 * where one bit identifies which set owns the element. This function returns
 * that bit's position for `set` within its group.
 *
 ...
 * filesystem. If the BPF objects can't be pinned, the program is unloaded and
 * the maps destroyed.
 *
