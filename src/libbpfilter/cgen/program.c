 ...
 *
 * One `bf_set_group` exists for every unique key format among a chain's
 * non-empty sets. The sets list preserves insertion order; a set's position
 * is its bit index within the group's bitmask value.
 */
struct bf_set_group
{
    /** Non-empty sets sharing the group's key format. Non-owning pointers
     * into the chain's `bf_set` list. Never empty. */
    bf_list sets;
...
static int _bf_program_build_set_groups(struct bf_program *program)
{
    ...
    program->set_groups =
        bf_list_default((bf_list_ops_free)_bf_set_group_free, NULL);
    ...
    bf_list_foreach (&program->set_groups, group_node) {
        ...
        bf_list_foreach (&group->sets, set_node) {
            ...
            if (bf_list_is_tail(&group->sets, set_node))
                bf_dump_prefix_last(prefix);
            DUMP(prefix, "bit %lu: bf_set '%s' (%lu element(s))", i, set->name,
 ...
 *
 * Sets that share the same key format have already been grouped by
 * `_bf_program_build_set_groups()`. Each group collapses to a single BPF map
 * whose value is a bitmask: bit `i` of byte `i / CHAR_BIT` identifies the
 * `i`-th set in the group.
 *
    ...
    bf_list_foreach (&new_prog->set_groups, group_node) {
        ...
        bf_list_foreach (&group->sets, set_node) {
            ...
            bf_list_foreach (&set->elems, elem_node) {
                ...
                r = bf_map_set_elem(new_map, elem, value);
                if (r)
