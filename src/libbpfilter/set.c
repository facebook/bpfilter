...
size_t bf_set_size(const struct bf_set *set)
{
    ...
    return bf_hashset_size(&set->elems);
}

const char *bf_set_name(const struct bf_set *set)
{
    assert(set);
...
size_t bf_set_n_comps(const struct bf_set *set)
{
    assert(set);
...
const enum bf_matcher_type *bf_set_key(const struct bf_set *set)
{
    assert(set);
...
int bf_set_add_many(struct bf_set *dest, struct bf_set **to_add)
{
    ...
    bf_set_foreach (*to_add, elem) {
        r = bf_set_add_elem(dest, elem);
        if (r)
...
int bf_set_remove_many(struct bf_set *dest, struct bf_set **to_remove)
{
    ...
    bf_set_foreach (*to_remove, elem)
        bf_hashset_remove(&dest->elems, elem);
