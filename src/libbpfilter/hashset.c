...
static int _bf_hashset_grow(struct bf_hashset *set)
{
    ...
    size_t new_n_slots = old_n_slots ? old_n_slots * 2 : _BF_HASHSET_INIT_CAP;
    size_t slot_size = _bf_slot_size(set->elem_size);
    struct bf_vector old_slots = TAKE_STRUCT(set->slots);
    uint8_t empty_slot[slot_size];
    int r;
    ...
    for (size_t i = 0; i < new_n_slots; ++i) {
        r = bf_vector_add(&set->slots, empty_slot);
        if (r) {
    ...
    for (size_t i = 0; i < old_n_slots; ++i) {
        ...
        size_t idx = _bf_hashset_hash(set, old_slot + sizeof(uint8_t));

        while (_bf_slot_status(set, idx) == _BF_SLOT_OCCUPIED)
            idx = (idx + 1) % _bf_n_slots(set);

        _bf_slot_set(set, idx, _BF_SLOT_OCCUPIED, old_slot + sizeof(uint8_t));
        ++set->len;
        ++set->n_used;
    }
...
static int _bf_hashset_parse_key(const char *raw_key,
                                 enum bf_matcher_type *key, size_t *n_comps)
{
...
int bf_hashset_add_elem(struct bf_hashset *set, const void *elem)
{
    ...
    return bf_hashset_add(set, elem);
}
...
int bf_hashset_add_many(struct bf_hashset *dest,
                        struct bf_hashset **to_add)
{
    int r;
    ...
    for (size_t i = 0; i < _bf_n_slots(*to_add); ++i) {
        if (_bf_slot_status(*to_add, i) != _BF_SLOT_OCCUPIED)
            continue;

        r = bf_hashset_add(dest, _bf_slot_data(*to_add, i));
        if (r)
            return r;
    }
                           ...
                           struct bf_hashset **to_remove)
{
    ...
    for (size_t i = 0; i < _bf_n_slots(*to_remove); ++i) {
        if (_bf_slot_status(*to_remove, i) != _BF_SLOT_OCCUPIED)
            continue;

        r = bf_hashset_remove(dest, _bf_slot_data(*to_remove, i));
        if (r)
            return r;
    }
