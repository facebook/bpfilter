                                      ...
                                      const char *raw_payload)
{
    ...
    if (endptr[0] == '%' && endptr[1] == '\0' && proba >= 0.0 && proba <= 100.0) {
        *(float *)payload = (float)proba;
        return 0;
    }

    bf_err(
        "\"%s %s\" expects a valid floating-point percentage value (i.e., within [0%%, 100%%]), not '%s'",
        bf_matcher_type_to_str(type), bf_matcher_op_to_str(op), raw_payload);
...
static void _bf_print_flow_probability(const void *payload)
{
    ...
    (void)fprintf(stdout, "%.2f%%", (double)*(float *)payload);
}
