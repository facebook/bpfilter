                        ...
                        const struct bf_set *to_remove)
{
    ...
    bf_wpack_kv_str(wpack, "chain_name", chain_name);
    ...
    if (to_remove) {
        ...
        bf_wpack_kv_nil(wpack, "to_remove");
    }

    r = bf_request_new_from_pack(&request, BF_FRONT_CLI, BF_REQ_CHAIN_SET_UPDATE,
                                 wpack);
    if (r)
        return bf_err_r(r, "bf_chain_set_update: failed to create a new request");
    ...
    r = bf_send(fd, request, &response, NULL);
    if (r)
        return bf_err_r(r, "bf_chain_set_update: failed to send request");
