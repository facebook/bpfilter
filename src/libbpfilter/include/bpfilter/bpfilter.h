 ...
 * {
 *   "chain_name": "",
 *   "to_add": { }, // bf_set object or nil
 *   "to_remove": { } // bf_set object or nil
 * }
 * @endcode
 *
 * **Response payload format**
 * The response doesn't contain data.
 *
 * @param chain_name Name of the chain containing the set. Can't be NULL.
 * @param to_add Set containing elements to add. The set name and key format
 *        must match the existing set in the chain. Can be NULL if no elements
 *        to add.
 * @param to_remove Set containing elements to remove. The set name and key
 *        format must match the existing set in the chain. Can be NULL if no
 *        elements to remove.
 * @return 0 on success, or a negative errno value on failure, including:
 * - `-ENOENT`: no chain found for this name or set not found in chain.
 * - `-EINVAL`: set key format doesn't match existing set.
 */
int bf_chain_update_set(const char *chain_name, const struct bf_set *to_add,
