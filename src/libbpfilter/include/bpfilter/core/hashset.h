...
/**
 * @brief A single slot in the hashset's backing array.
 */
typedef struct bf_hashset_elem
{
    /// Data pointer, tombstone, or NULL.
    void *data;
 ...
 * @param ctx_ptr Opaque context pointer passed to every callback. Can be NULL.
 ...
 * Safe to remove the current element during iteration.
 *
 ...
 *
 * Ensures the backing array is large enough to hold @p count elements without
 ...
 * On successful insertion, the hashset takes ownership of `data` and
 * appends it to the end of the iteration order.
 *
